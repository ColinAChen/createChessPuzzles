...
class VariationalEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.linear1(x))
        mu = self.linear2_1(x)
        sigma = torch.exp(self.linear2_2(x))
...
def train_vae(vae, data, epochs=20):
    ...
    for epoch in range(epochs):
        ...
        for x in data:
            ...
            opt.zero_grad()
            x_hat = vae(x)
            loss = ((x - x_hat)**2).sum() + vae.encoder.k1
            loss.backward()
            opt.step()
    return vae
...
def test():
    ...
    autoencoder = train(autoencoder, data, epochs=5)
    torch.save(autoencoder, SAVE_DIR)


if __name__ == "__main__":
    SAVE_DIR = "./saved_models/mnist_vae.pkl"
    data = torch.utils.data.DataLoader(
        torchvision.datasets.MNIST('./data', transform=torchvision.transforms.ToTensor(),download=True),
        batch_size=128,
        shuffle=True
